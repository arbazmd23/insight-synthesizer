...
class SurveyAnalyzer:
    ...
    def load_model(self):
        ...
        self.llm = Llama(
            ...
            n_gpu_layers=0,  # Change if you have GPU support
...
"""
    ...
    def extract_single_theme(self, responses):
        """Extract a single dominant theme from model output with fallback"""
        ...
        try:
            # Try with timeout on Unix-like systems
            try:
            ...
            dominant_theme = self.parse_single_theme(output['choices'][0]['text'], responses)
            ...
            if dominant_theme:
                ...
                print("Failed to parse theme from model output. Using fallback.")
                return self.fallback_single_theme_extraction(responses)
    ...
    def parse_single_theme(self, output_text, all_responses):
        ...
        # Debug: Print sample of the output
    ...
    def fallback_single_theme_extraction(self, responses):
        """Extract a single dominant theme as a fallback method"""
        ...
        if not potential_themes:
            ...
            sentiment_counts = Counter([self.analyze_sentiment(r) for r in responses])
            dominant_sentiment = sentiment_counts.most_common(1)[0][0]
            
            return {
                "theme": f"General {dominant_sentiment.title()} Feedback",
                "quotes": [r for r in responses if self.analyze_sentiment(r) == dominant_sentiment][:5],
        ...
        sentiments = [self.analyze_sentiment(q) for q in best_theme["quotes"]]
    ...
    def generate_output_and_visualization(self, dominant_theme, responses):
        ...
        # Create final output - without redundant fields
        output = {
...
def load_responses():
    ...
    if os.path.exists('cleaned_survey_responses_testing.csv'):
